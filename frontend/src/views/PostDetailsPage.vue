...
const onDeleteButtonClick = async () => {
  try {
    if (post.value) {
      ...
      modalStore.openModal(DeletePostConfirmationForm, 'Confirm Post Deletion', {
        post: postToDelete
      })
    }
  } catch (error) {
...
<template>
  <div v-if="post" class="card my-5">
    ...
    <footer v-if="authStore.isUserLoggedIn && (canEdit || canDelete)" class="card-footer">
      <a v-if="canEdit" @click="onEditButtonClick" class="card-footer-item">Edit</a>
      <a v-if="canDelete" @click="onDeleteButtonClick" class="card-footer-item">Delete</a>
    </footer>
  </div>
  <div v-if="!post">
